...
本次已直接修改用户提供的 `定制版巨魔/TrollInstallerX` 工作目录，并构建了远程版 0.8.0 / build 16。该目录有既有未提交定制内容，且 origin 指向第三方上游，因此不向该 upstream 推送；本仓库保存此次变更补丁和基线摘要。
...
3. 在 PHP 服务项目执行 `php bin/export-installer.php --url https://mtx.jk92.cc --out /工程完整路径/TrollInstallerX/TrollInstallerX/Installer/MTXRemote.generated.h`，使用与生产一致的私有配置。该命令只输出公钥和固定公开绑定，不导出私钥。
