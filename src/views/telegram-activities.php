...
<div id="trial-stock-table" data-total="<?=$stockTotal?>" data-page="<?=$stockPage?>" data-url="<?=e($action.'&stock_status='.rawurlencode($stockStatus))?>"></div>
...
<div id="trial-stock-fallback">
<?php if (!$stock): ?><div class="empty-state"><h3>暂无符合条件的卡密</h3><p>请导入卡密或切换库存状态查看。</p></div><?php else: ?>
<div class="bot-table-wrap"><table class="bot-table" id="trial-stock-source"><thead><tr><th>选择</th><th>卡密</th><th>状态</th><th>领取用户</th><th>操作</th></tr></thead><tbody>
...
<?php endif ?>
<nav class="bot-actions" aria-label="库存分页"><?php if ($stockPage>1): ?><a class="button quiet" href="<?=e($stockUrl($stockPage-1))?>">上一页</a><?php endif ?><span class="muted">第 <?=$stockPage?> / <?=$stockPages?> 页</span><?php if ($stockPage<$stockPages): ?><a class="button quiet" href="<?=e($stockUrl($stockPage+1))?>">下一页</a><?php endif ?></nav></div></section>
<section class="panel bot-records"><div class="section-title"><h2>领取记录</h2><span class="muted">本活动最近 100 条 · 原始卡密可选中复制</span></div>
<div id="trial-claims-table"></div><div id="trial-claims-fallback">
<?php if (!$records): ?><div class="empty-state"><h3>还没有领取记录</h3><p>用户点击领取后，这里显示数字 ID、领取日期、原始卡密和发送状态。</p></div><?php else: ?><div class="bot-table-wrap"><table class="bot-table" id="trial-claims-source"><thead><tr><th>领取时间（北京时间）</th><th>用户 ID</th><th>原始卡密</th><th>发送状态</th></tr></thead><tbody><?php foreach ($records as $hash=>$card): ?><tr><td><?=e((new DateTimeImmutable('@'.$card['at']))->setTimezone(new DateTimeZone('Asia/Shanghai'))->format('m-d H:i:s'))?></td><td>#<?=e($card['peer'])?></td><td><input type="text" readonly autocomplete="off" aria-label="领取记录原始卡密" value="<?=e($card['code'])?>" style="min-width:220px;width:100%;font-family:monospace"></td><td><?=e($trialLabels[$card['status']]??$card['status'])?></td></tr><?php endforeach ?></tbody></table></div><?php endif ?>
</div><p class="muted compact">“已发送”代表 Telegram 接收成功，不代表用户已读或卡片已激活。保留数字 ID 是为了限领和核对，不记录用户名或聊天内容。</p></section>
