/* Based on TrollStore fastPathSign at 88424f683b2a08f34a3f88985f790f97d84ce1df; see vendor/LICENSE-TrollStore. Platform-only XML serialization change. */
...
#include "Base64.h"
#include "vendor/templates/AppStoreCodeDirectory.h"
#include "vendor/templates/DERTemplate.h"
#include "vendor/templates/TemplateSignatureBlob.h"
#include "vendor/templates/CADetails.h"
...
int update_signature_blob(CS_DecodedSuperBlob *superblob)
{
    ...
    /* Portable serialization of the same two cdhash values. */
    unsigned char hash1[29], hash2[29];
    EVP_EncodeBlock(hash1, sha1CDHash, 20);
    EVP_EncodeBlock(hash2, sha256CDHash, 20);
    char xml[512];
    int n = snprintf(xml, sizeof(xml), "<?xml version=\"1.0\" encoding=\"UTF-8\"?><!DOCTYPE plist PUBLIC \"-//Apple//DTD PLIST 1.0//EN\" \"http://www.apple.com/DTDs/PropertyList-1.0.dtd\"><plist version=\"1.0\"><dict><key>cdhashes</key><array><data>%s</data><data>%s</data></array></dict></plist>", hash1, hash2);
    if (n < 0 || n >= sizeof(xml) || !CMS_signed_add1_attr_by_txt(newSigner, "1.2.840.113635.100.9.1", V_ASN1_OCTET_STRING, (unsigned char *)xml, n)) return -1;
