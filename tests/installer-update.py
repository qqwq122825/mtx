...
with tempfile.TemporaryDirectory(prefix='mtx-native-client-') as temp:
    ...
    values={'MTX_REMOTE_GAME_ID':1,'MTX_REMOTE_GAME_NAME':'满天星三角洲国服','MTX_REMOTE_UPDATE_URL':'https://fixture.invalid/r-fixture/api/update.php','MTX_REMOTE_PROFILE':'mtx-dfm-remote-v1','MTX_REMOTE_BUNDLE_ID':'com.mtx.scmtxdfm','MTX_REMOTE_KEY_ID':'fixture-1','MTX_REMOTE_PUBLIC_KEY':(t/'pub').read_text()}
...
@main struct ClientTest {
 static func main() {
    ...
    DispatchQueue.global().async {
        ...
        if mode == "legacy_sequence" { defaults.set(999,forKey:"MTXRemoteMaxSequence") }
...
'''.replace('let suite="MTXFixture-"+UUID().uuidString, defaults=UserDefaults(suiteName:"MTXFixture-"+UUID().uuidString)!','let suite="MTXFixture-"+UUID().uuidString\n        let defaults=UserDefaults(suiteName:suite)!'))
    ...
    class Handler(http.server.BaseHTTPRequestHandler):
        ...
        def do_GET(self):
            ...
            if p.path.endswith('update.php'):
                ...
                release={'release_id':('b' if mode=='switched' and update_calls>=2 else 'a')*32,'sequence':1,'bundle_id':'com.fixture.other' if mode=='wrong_game' else 'com.mtx.scmtxdfm','profile_id':'mtx-dfm-remote-v1','artifact_format':'prepared-payload-v1','artifact_sha256':sha(tar),'artifact_bytes':len(tar),'source_sha256':sha(source),'source_bytes':len(source),'manifest_sha256': '0'*64 if mode=='manifest_hash' else manifest_sha,'min_ios':'14.0.0','max_ios':'16.6.1','min_installer_version':'0.8.0'}
                now=time.time();stamp=lambda n:datetime.fromtimestamp(n,timezone.utc).isoformat(timespec='seconds')
                payload={'schema_version':1,'game_id':(2 if mode=='wrong_id' else True if mode=='boolean_id' else '1' if mode=='string_id' else 1),'nonce':'wrong' if mode=='wrong_nonce' else q['nonce'][0],'issued_at':stamp(now-700 if mode=='expired' else now),'expires_at':stamp(now-100 if mode=='expired' else now+600),'status':'no_release' if mode=='no_release' else 'version_unknown','release':None if mode=='no_release' else release}
                if mode=='missing_id': payload.pop('game_id'); payload['app_key']='mtx-dfm-cn'
                self.send(sign(payload))
        ...
        def do_POST(self):
            ...
            ticket=data|{'game_id':2 if mode=='wrong_ticket_id' else 1,'url':url}
            if mode=='legacy_ticket': ticket.pop('game_id'); ticket['app_key']='mtx-dfm-cn'
            self.send(json.dumps(ticket).encode())
    ...
    try:
        for mode in ['success','sibling_sequence','scoped_rollback','source_corrupt','source_short','bad_signature','wrong_id','boolean_id','missing_id','string_id','wrong_ticket_id','legacy_ticket','wrong_nonce','wrong_game','expired','no_release','redirect','external_ticket','manifest_hash','traversal','legacy_sequence','switched']:
            ...
            check(report['ok']==(mode in ['success','sibling_sequence','legacy_sequence']),'native validation result: '+mode+' '+report['detail'])
            if mode in ['success','sibling_sequence','legacy_sequence']:
