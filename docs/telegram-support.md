...
- `/start`（含管理员自己的账号）显示「你好，这里是满天星 ✨ 请直接发送你的问题、图片或文件，看到消息后我会尽快回复你」及三个快捷按钮。管理员 `/help` 仍显示客服管理指令。
- 三个按钮为「💬 项目咨询」「🛠 安装帮助」「📮 售后反馈」。使用 Telegram 官方 [ReplyKeyboardMarkup](https://core.telegram.org/bots/api#replykeyboardmarkup) 普通文字键盘，不新增回调权限；按钮只给当前用户发送对应提示，用户继续发送实际问题后照常转发给管理员。管理员回复用户时即便正文与按钮相同，也仍作为人工回信转发。
