...
打开 `/admin/` 会进入密码登录页，不需要用户名。登录页默认勾选“记住登录 30 天”，浏览器只持有随机会话 ID（生产环境为 Secure、HttpOnly、SameSite=Strict Cookie），密码和密码摘要不会写入网页缓存或 localStorage。会话数据仍保存在非公开的 `storage/sessions` 文件夹，不需要数据库。

30 天为本次登录的固定有效期，日常访问不会自动延长；取消勾选时使用浏览器会话 Cookie，服务端有效期为 8 小时。退出登录会删除 Cookie 并销毁服务端会话，更换密码会让所有旧会话失效。公共设备请取消勾选；清除站点 Cookie 后需要重新输入密码。

部署时保留应用专用会话目录，不要给它套用短于 30 天的外部清理任务。应用已为该目录设置 PHP 会话回收保留期；登录是否过期仍由服务端时间和当前密码摘要共同校验。
...
当前服务不执行 TIPA 内程序、不编译巨魔。管理员只上传辅助 TIPA，服务器 Python 队列使用固定的处理组件生成内部 TAR，并校验源摘要、Bundle ID、显示版本、程序代码、权限及文件摘要。自动处理组件需要首次编译并配置每分钟运行，步骤见 [单包自动处理](../docs/automatic-preparation.md)。没有配置任务时，上传停留在排队状态。
