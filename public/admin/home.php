...
?>
<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>主页设置 · 满天星</title><link rel="stylesheet" href="<?=e($app->path('/assets/app.css'))?>"></head><body>
<header class="topbar"><a class="brand" href="<?=e($app->path('/admin/'))?>">✳ 满天星</a><a href="<?=e($app->path('/admin/'))?>">返回后台</a></header>
<main class="content bot-page"><div class="page-heading"><div><div class="eyebrow">WEBSITE SETTINGS</div><h1>主页设置</h1><p class="muted">配置主页的买卡与安装器下载入口，无需修改代码。</p></div><a class="button quiet" href="/" target="_blank" rel="noopener">查看主页 ↗</a></div>
...
<section class="panel"><form method="post" action="<?=e($app->path('/admin/home.php'))?>" class="bot-form">
...
<section class="panel bot-records"><h2>按游戏上传安装器</h2><p class="muted">只上传编译完成的安装器，不是辅助更新包。每个游戏保留一个当前下载入口；上传新版后自动切换，不删除旧文件。上传上限 <?=e(round($app->config['max_upload_bytes']/1048576))?> MiB。</p>
...
<h3>当前安装器</h3><?php foreach ($state['apps'] as $game): $installer=$state['home_installers'][(string)$game['game_id']]??null; ?><p><?=e($game['name'])?> · <?= $installer?e(number_format($installer['bytes']/1048576,2)).' MiB':'尚未上传'?><?php if ($installer): ?> · <a href="/installer?game_id=<?=e($game['game_id'])?>">下载安装器</a><?php endif ?></p><?php endforeach ?>
