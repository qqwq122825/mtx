...
PHP 后台 + 服务器目录存储，**没有数据库、Redis 或前端构建服务**。版本及审计写入 JSON，包文件按 SHA-256 保存；密码校验摘要和发布密钥在服务器私有 PHP 配置中。

## 已实现

- 后台默认 `/admin/`，直接重命名 public 下的后台文件夹即可变更入口；无用户名，仅密码登录。
- 安装器 API 保留固定随机前缀，主页和旧后台路径 404，不泄露新后台地址。
- 单管理员密码登录、CSRF、默认记住登录 30 天 / 非记住模式 8 小时、错误密码限流、退出登录。
- 游戏 ID 自动分配、统一接口地址、预期 Bundle ID、启用/停用。
- TIPA 上传进度、XML/binary plist 元数据识别、ZIP 路径/大小/CRC/身份检查。
- 只上传匹配的辅助 TIPA，服务器自动校验并生成安装资源；显示排队、处理、失败重试及待发布状态。
...
**客户端已接入：定制版巨魔 0.8.3 / build 19，先下载 TIPA（字节进度条）→ 下载匹配的 TAR → 校验 → 准备缓存 → 安装。** 下载错误只显示重试提示，安装阶段异常才显示诊断日志。已完成本地自动测试、iOS 构建、正式服务器部署和客户端生产接口验签；正式站当前尚未发布游戏包，完整线上下载与真机安装仍待验证。

后台只上传一个辅助 TIPA；服务器 Python 队列调用固定的本地处理组件，自动生成内部安装资源 TAR，不执行包内 iOS 程序，不需要 Mac 常驻。对外下载契约保持不变，当前远程安装器无需重新打包。见 [单包自动处理部署](docs/automatic-preparation.md)。固定游戏客户端的具体接入、CDN 规则和密钥配置见 [远程安装器与 CDN](docs/cdn-and-installer.md)。
...
- 自动包处理：Python 3.9+、Clang、OpenSSL 开发库；Linux 还需要 BlocksRuntime，安装与定时任务见自动处理部署说明。
...
1. 登录后台，选择游戏；新增游戏只填名称、真实 Bundle ID，后台自动分配数字 ID 和内部产物契约。现有三角洲为 ID `1`。
2. 选择 TIPA 上传即可；备注可选、仅后台可见。包信息自动读取，版本兼容参数由程序内部维护。
3. 服务器自动处理；通常下一分钟内开始。点击“刷新处理状态”查看结果；失败时检查提示并重试，无需再上传 TAR。
...
HTTP 集成测试使用 Python 3 标准库，自动创建临时 PHP 多进程服务器与合成测试包，结束后清理，不触及正式 storage。Node 仅用于上传控制器回归测试，运行服务不需要 Node。自动处理测试另见 `tests/preparation.py`，需先编译处理组件并指定实际匹配的辅助 TIPA。
