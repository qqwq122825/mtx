...
$s=$app->store->read(); $g=GameIds::resolve($s,$body,'app_key'); $key=$g['app_key']; $r=Releases::release($s,$id,$key);
...
Http::json(['game_id'=>$g['game_id'],'app_key'=>$key,'release_id'=>$id,'artifact_id'=>$sha,'url'=>$app->url('/api/download.php?'.$query),'expires_at'=>gmdate('c',$expiry)]);
