...
回信实际送达后，管理员收到「✅ 已回复 @用户名（#数字ID）」。只对已确认送达的回复，按关联的数字 ID 调用 `getChat` 获取当前称呼；无用户名时使用昵称，查询失败或返回身份不匹配时直接显示数字 ID，不重发已送达的回信，不把查询结果存入服务器。成功回执不表示用户已读。

新消息卡片正常通知，正文副本和管理员成功回执通过 [disable_notification](https://core.telegram.org/bots/api#sendmessage) 静默发送，减少同一条咨询的重复声音。静默消息仍会显示；实际声音/弹窗受 Telegram 会话及系统通知设置影响。用户收到人工回信的通知不变；异常提示保留正常通知。
...
活动库存另外保存于同一个私有状态的 `trials` 区域，包含待发卡密和已分配卡原文，用于当天取回；后台不回显完整卡密。删除活动会清除原文，已分配卡指纹与近期每日限领记录仍保留。更换机器人绑定保留此区域但自动暂停活动。
