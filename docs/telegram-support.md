...
后台「Telegram 客服 → 默认自动回复」可直接编辑五段纯文字：开始欢迎语、项目咨询、安装帮助、售后反馈、消息收到提示。每段 1–1000 字，支持换行，保存后对新消息生效，无需暂停或重设 Token。原配置缺少该项时直接使用默认值；第一次绑定保留预先编辑的文案，更换已有机器人身份则重置为默认文案。

- `/start`（含管理员自己的账号）显示「你好，这里是满天星 ✨ 请直接发送你的问题、图片或文件，看到消息后我会尽快回复你」及三个消息内按钮。管理员 `/help` 仍显示客服管理指令。
- 三个按钮为「💬 项目咨询」「🛠 安装帮助」「📮 售后反馈」。使用 Telegram 官方 [InlineKeyboardMarkup](https://core.telegram.org/bots/api#inlinekeyboardmarkup)，按钮附在欢迎消息下方，不占用输入框；通过 `callback_query` 接收点击，只给当前用户发送对应提示，用户继续发送实际问题后照常转发给管理员。管理员回复用户时即便正文与按钮相同，也仍作为人工回信转发。
...
- 已接入旧版的机器人，在后台展开「卡片按钮接入维护」，点击一次「更新消息订阅」。保留原 Token、Webhook secret、会话和待处理消息，无需暂停。之后发送 `/start` 获取新卡片；旧历史消息不批量修改。普通回复会移除先前的输入框键盘（管理员也可先发送 `/help` 清理）。
- 卡片点击只接受本人私聊里由当前机器人发送的消息，按钮数据仅允许三个固定类型；群组、其他机器人、陌生数据均忽略。点击沿用屏蔽、限流、去重与重试机制，调用 `answerCallbackQuery` 结束加载提示，随后私聊发出对应文案，不转发成新的客服问题。卡片原文不落盘。
