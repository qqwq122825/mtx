...
if (!$g['enabled'] || $g['current_release_id']!==$id || $r['state']!=='published' || $sha!==$r['artifact_sha256']) throw new Problem(404,'此版本已停止下载。');
$path=$app->object($sha);
$h=@fopen($path,'rb');
if (!$h) throw new Problem(404,'发布文件不存在。');
$stat=fstat($h); $size=$stat['size'];
if ($size!==$r['artifact_bytes']) { fclose($h); throw new Problem(503,'发布文件长度异常。'); }
...
header('Content-Disposition: attachment; filename="'.$g['app_key'].'-'.$r['sequence'].($r['artifact_format']==='tipa'?'.tipa':'.tar').'"');
