...
<p class="muted compact">点击“查看卡密”展开完整内容。勾选后可批量删除未分配库存；已分配卡保留用于用户再次取卡。删除库存不影响活动设置，也不撤回 Telegram 消息。</p>
...
<div class="bot-table-wrap"><table class="bot-table"><thead><tr><th>选择</th><th>卡密</th><th>状态</th><th>领取用户</th><th>操作</th></tr></thead><tbody>
<?php foreach ($stock as $hash=>$card): ?><tr><td><?php if ($card['status']==='available'): ?><input type="checkbox" name="cards[]" value="<?=e($hash)?>" form="stock-delete" aria-label="选择卡密 <?=e(substr($hash,0,12))?>"><?php else: ?>—<?php endif ?></td><td><details><summary>查看卡密 · <?=e(substr($hash,0,12))?></summary><input type="text" readonly autocomplete="off" aria-label="完整卡密" value="<?=e($card['code'])?>" style="min-width:220px;width:100%;font-family:monospace"></details></td><td><?=e($trialLabels[$card['status']]??$card['status'])?></td><td><?=$card['peer']?'#'.e($card['peer']):'—'?></td><td><?php if ($card['status']==='available'): ?><button class="text-button danger" type="submit" name="card" value="<?=e($hash)?>" form="stock-delete">删除此卡</button><?php else: ?><span class="muted">已分配 · 保留</span><?php endif ?></td></tr><?php endforeach ?>
</tbody></table></div><div class="bot-actions"><button class="text-button danger" type="submit" form="stock-delete">删除勾选卡密</button></div>
<?php endif ?>
